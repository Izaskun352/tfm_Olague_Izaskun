

source("scripts/00_setup.R")

#------------------------------------------------------------------------------------------------------
# ORAL  x PERIPHERAL NEUROPATHY / AUTISM SPECTRUM / INSOMNIA --> EPIGENETIC REGULATION / HETEROCHROMATIN
#------------------------------------------------------------------------------------------------------

# INPUTS
lista_intersecciones_np <- readRDS("./Output/Piloto_Microbiota/lista_intersecciones_NP_MicroGWAS.rds")
lista_intersecciones_im <- readRDS("./Output/Piloto_Microbiota/lista_intersecciones_IM_MicroGWAS.rds")
traits_MicroGWAS_areas <- readRDS("./Data/Diccionarios/traits_MicroGWAS_areas.rds")

# OUTPUTS

readRDS("./Output/Redes_Capas/Tablas/oral_pneuropathy.rds" )
readRDS( "./Output/Redes_Capas/Tablas/oral_autism.rds" )
readRDS( "./Output/Redes_Capas/Tablas/oral_insomnia.rds" )
readRDS("./Output/Redes_Capas/Tablas/megacluster_epigenetic.rds" )

# CARGAR LO COMÚN

load("Data/Redes/config_red.RData")               # colores_db, prioridad_db, add_layer
aristas_color <- readRDS("Data/Redes/aristas_color.rds")
source("Scripts/Scripts_Limpios/Red_Capas_Finales/01_Funciones_Redes.R")


#------------------------------------------------------------------------------------------------------
# ESTUDIAR SOLAPAMIENTO
#------------------------------------------------------------------------------------------------------

genes_oral_per_neuropathy <- lista_intersecciones_np[[22]]$genes_ensembl  # 203
genes_tabla <- lista_intersecciones_np[[22]]$genes_tabla
write.csv2(genes_tabla, file = "./Output/Redes_Capas/Tablas/oral_pneuropathy.csv" )
saveRDS(lista_intersecciones_np[[22]]$genes_tabla, file = "./Output/Redes_Capas/Tablas/oral_pneuropathy.rds" )

genes_oral_autism <- lista_intersecciones_np[[23]]$genes_ensembl  # 151
write.csv2(lista_intersecciones_np[[23]]$genes_tabla, file = "./Output/Redes_Capas/Tablas/oral_autism.csv" )
saveRDS(lista_intersecciones_np[[23]]$genes_tabla, file = "./Output/Redes_Capas/Tablas/oral_autism.rds" )

genes_oral_insomnia <- lista_intersecciones_np[[24]]$genes_ensembl  # 171
write.csv2(lista_intersecciones_np[[24]]$genes_tabla, file = "./Output/Redes_Capas/Tablas/oral_insomnia.csv" )
saveRDS(lista_intersecciones_np[[24]]$genes_tabla, file = "./Output/Redes_Capas/Tablas/oral_insomnia.rds" )

genes_compartidos <- Reduce(intersect, list(genes_oral_autism, genes_oral_insomnia, genes_oral_per_neuropathy))  # 124 compartidos 

genes_gut_IBD <- lista_intersecciones_im[[3]]$genes_ensembl
genes_oral_psoriasis <- lista_intersecciones_im[[2]]$genes_ensembl
genes_oral_eczema <- lista_intersecciones_im[[1]]$genes_ensembl
genes_oral_lupus <- lista_intersecciones_im[[8]]$genes_ensembl
genes_oral_rhinitis <- lista_intersecciones_im[[7]]$genes_ensembl

elementos <- list(
  lista_intersecciones_np[[23]],
  lista_intersecciones_np[[24]],
  lista_intersecciones_np[[22]]  # añade los que necesites
  # lista_intersecciones_im[[1]],
  # lista_intersecciones_im[[2]],
  # #lista_intersecciones_im[[3]],
  # lista_intersecciones_im[[7]],
  # lista_intersecciones_im[[8]]
)
genes_compartidos <- Reduce(intersect, lapply(elementos, function(x) x$genes_ensembl))  # 103 genes

tabla_compartidos <- lapply(elementos, function(inter) {
  inter$genes_tabla %>%
    dplyr::filter(gene.ENSG %in% genes_compartidos) %>%
    dplyr::mutate(
      par_origen = inter$c1,
      trait_1    = inter$trait_1,
      trait_2    = inter$trait_2
    )
}) %>%
  dplyr::bind_rows() %>%
  dplyr::group_by(gene.ENSG, gene.gene) %>%
  dplyr::summarise(
    es_semilla_en_alguno = any(es_semilla_en_alguno),
    n_pares_presente     = dplyr::n(),
    pares                = paste(par_origen, collapse = ";"),
    .groups = "drop"
  )
write.csv2(tabla_compartidos, file = "./Output/Redes_Capas/Tablas/megacluster_epigenetic.csv" )
saveRDS(tabla_compartidos, file = "./Output/Redes_Capas/Tablas/megacluster_epigenetic.rds" )

# Construir grafo base

genes_tabla_merged <- bind_rows(
 lista_intersecciones_np[[22]]$genes_tabla,lista_intersecciones_np[[23]]$genes_tabla,lista_intersecciones_np[[24]]$genes_tabla
 # ,lista_intersecciones_im[[1]]$genes_tabla, 
 #  lista_intersecciones_im[[2]]$genes_tabla, 
 #  lista_intersecciones_im[[8]]$genes_tabla, lista_intersecciones_im[[7]]$genes_tabla 
 ) %>%
  dplyr::filter(gene.ENSG %in% genes_compartidos)

# Objeto compatible con build_network
interseccion_compartida <- list(
  genes_ensembl = genes_compartidos,
  genes_tabla   = genes_tabla_merged)

g <- build_network(interseccion_compartida, aristas_color, colores_db)

# Añadimos genes semilla

g <- add_layer_seed(g, interseccion_compartida$genes_tabla)
#--------------------------------------------------------
# AÑADIMOS CAPAS / ATRIBUTOS
#--------------------------------------------------------

# ---- GENES DIFERENCIALMENTE EXPRESADOS EN ENFERMEDADES - OTAR  ----

lista_genes_DEG_np <- readRDS("./Output/Piloto_Microbiota/Expresion_Diferencial_OTAR/df_genes_NP.rds")
genes_DEG_np <- lista_genes_DEG_np$targetId
intersect(genes_DEG_np, genes_compartidos)

g <- add_layer_DEG(g, lista_genes_DEG_np)

# ---- GENES QUE SON DRUG TARGETS ----

intersect(chmbl$targetId, genes_compartidos)
g <- add_layer_drug_target(g, chmbl)

# ---- GENES CON VARIANTES RARAS DE INTERES ----

diseases_mapping <- readRDS("./Data/Diccionarios/diseases_mapping.rds")
variantes_raras_codificantes <- readRDS("./Data/Variantes/variantes_raras_codificantes.rds")

areas_interes <- c(
  "EFO_0000618",  # nervous system disease
  "MONDO_0002025") # psychiatric disorder

# IDs de enfermedad que pertenecen a esas áreas
ids_enfermedades_interes <- diseases_mapping %>%
  dplyr::filter(therapeuticArea %in% areas_interes) %>%
  dplyr::pull(id) %>%
  unique()
message("Enfermedades en áreas de interés: ", length(ids_enfermedades_interes))


# Cuántas variantes raras de tus genes caen en enfermedades de interés
variantes_raras_codificantes %>%
  as.data.frame(stringsAsFactors = FALSE) %>%
  dplyr::filter(
    targetFromSourceId        %in% V(g)$name,
    diseaseFromSourceMappedId %in% ids_enfermedades_interes
  ) %>%
  dplyr::summarise(
    n_variantes = n(),
    n_genes     = n_distinct(targetFromSourceId)
  ) # 1810 variantes en 36 genes de la red

# Añadimos la capa
g <- add_layer_rare_variants(g, variantes_raras_codificantes, ids_enfermedades_interes)
table(V(g)$has_rare_variant)
V(g)$name[V(g)$has_rare_variant == TRUE] # Qué genes son


# ---- ESENCIALIDAD GENËTICA (CRISPR KO)  ----
depmap_essential <- readRDS("./Data/CRISPR_KO/depmap_essential_por_grupo.rds")
crisprbrain_essential <- readRDS("./Data/CRISPR_KO/crisprbrain_essential.rds")

g <- add_layer_essential(g, depmap_essential, crisprbrain_essential)

#--------------------------------------------------------
# DIBUJAR GRAFO
#--------------------------------------------------------

# ---- Resumen de atributos de nodo ----
data.frame(
  name              = V(g)$name,
  is_DEG            = V(g)$is_DEG,
  is_drug_target    = V(g)$is_drug_target,
  has_rare_variant  = V(g)$has_rare_variant
) %>%
  dplyr::summarise(
    n_total          = n(),
    n_DEG            = sum(is_DEG, na.rm = TRUE),
    n_drug_target    = sum(is_drug_target, na.rm = TRUE),
    n_rare_variant   = sum(has_rare_variant, na.rm = TRUE),
    n_todas_capas    = sum(is_DEG & is_drug_target & has_rare_variant, na.rm = TRUE),
    n_DEG_y_drug     = sum(is_DEG & is_drug_target, na.rm = TRUE),
    n_DEG_y_rare     = sum(is_DEG & has_rare_variant, na.rm = TRUE),
    n_drug_y_rare    = sum(is_drug_target & has_rare_variant, na.rm = TRUE),
    n_ninguna        = sum(!is_DEG & !is_drug_target & !has_rare_variant, na.rm = TRUE)
  )

# ver esencialidad genética (CRISPR KO)
data.frame(
  name                   = V(g)$name,
  symbol                 = V(g)$symbol,
  is_essential_neural    = V(g)$is_essential_neural,
  is_essential_digestive = V(g)$is_essential_digestive,
  is_essential_immune    = V(g)$is_essential_immune,
  is_essential_other     = V(g)$is_essential_other,
  is_essential_neuron    = V(g)$is_essential_neuron
) %>%
  dplyr::mutate(
    n_grupos_essential = .data$is_essential_neural + 
      .data$is_essential_digestive +
      .data$is_essential_immune + 
      .data$is_essential_other,
    is_housekeeping    = n_grupos_essential == 4
  ) %>%
  dplyr::filter(is_housekeeping) %>%
  dplyr::select(symbol, is_essential_neuron, n_grupos_essential)
#n_neural n_digestive n_immune n_other n_neuron
#    22          24       28      23        7

# PLOT

plot <- plot_network(g, titulo = "Oral x Opioid Dependence", 
                     colores_db = colores_db, 
                     exclude_db = c("string", "signor", "reactome"),
                     use_qgraph_layout = TRUE)

cairo_pdf("./Output/Redes_Capas/Oral-Epigenetic_definitiva.pdf", width = 12, height = 12)
print(plot)
dev.off()

plot <- plot_network(g, titulo = "Oral x Opioid Dependence", 
                     colores_db = colores_db, 
                     exclude_db = c("string", "signor", "reactome"),
                     use_qgraph_layout = TRUE,
                     remove_unannotated = TRUE)

cairo_pdf("./Output/Redes_Capas/Oral-Epigenetic_immune.pdf", width = 14, height = 12)
print(plot)
dev.off()

# Eliminamos los nodos que no tienen atributos de las capas

plot_filtrado <- plot_network(g, titulo = "Oral x Opioid Dependence", 
                              colores_db = colores_db, 
                              exclude_db = c("string", "signor", "reactome"),
                              use_qgraph_layout = FALSE,
                              remove_unannotated = TRUE)

cairo_pdf("./Output/Redes_Capas/B_Oral-Epigenetic_NErvous.pdf", width = 15, height = 15)
print(plot_filtrado)
dev.off()

# Grafo incluyendo traits Immune

plot <- plot_network(g, titulo = "Epigenetic Supercluster", 
                     colores_db = colores_db, 
                     #exclude_db = c("string"), #, "signor", "reactome"),
                     use_qgraph_layout = TRUE,
                     remove_unannotated = TRUE)

cairo_pdf("./Output/Redes_Capas/B_Epigenetic_IM_NP.pdf", width = 12, height = 10)
print(plot)
dev.off()

plot_filtrado <- plot_network(g, titulo = "Epigenentic supercluster", 
                              colores_db = colores_db, 
                              exclude_db = c("string"), #, "signor", "reactome"),
                              use_qgraph_layout = FALSE,
                              remove_unannotated = TRUE)

cairo_pdf("./Output/Redes_Capas/B_Epigenetic_IM_NP_Filtrado.pdf", width = 30, height = 25)
print(plot_filtrado)
dev.off()

# Grafo incluyendo tissue expression atlas

plot_filtrado <- plot_network(g, titulo = "Epigenentic supercluster", 
                              colores_db = colores_db, 
                              exclude_db = c("string"), #, "signor", "reactome"),
                              use_qgraph_layout = FALSE,
                              remove_unannotated = TRUE)
cairo_pdf("./Output/Redes_Capas/B_Epigenetic_IM_NP_Expression_Atlas.pdf", width = 30, height = 25)
print(plot_filtrado)
dev.off()


# ANALISIS
atributos_nodos <- as.data.frame(vertex_attr(g))
map_variantes <- atributos_nodos %>%
  dplyr::filter(has_rare_variant == TRUE) %>%
  dplyr::select(name, Symbol_Red = symbol) %>% 
  dplyr::inner_join(as.data.frame(variantes_raras_codificantes), by = c("name" = "targetFromSourceId")) %>%
  dplyr::select(Gen = name, Symbol = Symbol_Red, Rasgo_ID = diseaseFromSourceMappedId) %>%
  dplyr::mutate(Capa = "Variante_Rara")

map_deg <- atributos_nodos %>%
  dplyr::filter(is_DEG == TRUE) %>% 
  dplyr::select(name, Symbol_Red = symbol) %>% 
  dplyr::inner_join(lista_genes_DEG_np, by = c("name" = "targetId")) %>%
  dplyr::select(Gen = name, Symbol = Symbol_Red, Rasgo_ID = diseaseId) %>% 
  dplyr::mutate(Capa = "DEG")

map_drugs <- atributos_nodos %>%
  dplyr::filter(is_drug_target == TRUE) %>% 
  dplyr::select(name, Symbol_Red = symbol) %>% 
  dplyr::inner_join(chmbl, by = c("name" = "targetId")) %>%
  dplyr::select(Gen = name, Symbol = Symbol_Red, Rasgo_ID = diseaseId) %>%
  dplyr::mutate(Capa = "Drug_Target")

#----------------------------------------------------------------------
# 3. Combinamos y Resumimos
#----------------------------------------------------------------------
mapeo_combinado <- dplyr::bind_rows(map_variantes, map_deg, map_drugs)

matriz_enfermedades_completa <- mapeo_combinado %>%
  dplyr::inner_join(traits_MicroGWAS_areas, by = c("Rasgo_ID" = "Rasgo")) %>%
  dplyr::group_by(Rasgo_ID, name, Nombre_area, therapeuticAreas) %>%
  dplyr::summarise(
    Genes_DEG          = paste(unique(Symbol[Capa == "DEG"]), collapse = ", "),
    Genes_DrugTarget   = paste(unique(Symbol[Capa == "Drug_Target"]), collapse = ", "),
    Genes_VarRaras     = paste(unique(Symbol[Capa == "Variante_Rara"]), collapse = ", "),
    Num_DEG            = n_distinct(Gen[Capa == "DEG"]),
    Num_DrugTarget     = n_distinct(Gen[Capa == "Drug_Target"]),
    Num_VarRaras       = n_distinct(Gen[Capa == "Variante_Rara"]),
    Total_Genes_Red    = n_distinct(Gen),
    .groups = "drop"
  ) %>%
  dplyr::rename(Enfermedad_Nombre = name) %>%
  dplyr::arrange(desc(Total_Genes_Red))

# Ver el resultado final
View(matriz_enfermedades_completa)


